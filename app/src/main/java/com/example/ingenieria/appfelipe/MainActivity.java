...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(picoYplacaHoy.equals(tv_placa.getText().toString())){
            ...
            //tvPicoProx.setText("En "+(j-diaAnio)+" días, "+date1);
            tv_proximo_dia.setText(date1);
            notificacion(date1);
        }

        else {
            //nos ubicamos en el primer dia del año que tuve pico y placa y le vamos sumando a ese dia 6 o 11 segun corresponda hasta el dia de hoy
            int posicion = Arrays.asList(PYP).indexOf(placaElegida);
            int j = 2 + posicion;

            while (j<diaAnio){

                j = localCalendar.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY ? j + 11 : j + 6;//si es Lunes suma 11 y si es otro dia suma 6
                localCalendar.set(Calendar.DAY_OF_YEAR, j);// cambia la fecha a la fecha sumada
            }
            Date date = localCalendar.getTime();
